...
class MapGenerator:
    ...
    def __placeEnt(self,RM,ent,duplicate=False):
        if not duplicate:
            ents = RM.findInside(self.ents,[RM])
            for a in ents:
    ...
    def __generate(self): #Generate the map
        ...
        for i,a in enumerate(Path[1:]):
            if random.randint(0,10)<self.__dif and random.randint(1,2)==2 and not a in Starting:
                ...
                if strat==1: #Move threat to secondary room
                    if not Empty is None:
                        self.ents.append(self.getEnt(threat)(a.pos[0]+(int(a.size[0]/100)*50),a.pos[1]+(int(a.size[1]/100)*50),self.__LINK,self.__IDCount))
                        self.__IDCount+=1
                    ...
                    if a.pos[0]==0 or a.pos[1]==0:
                        if a.pos[0]==0:
                            self.ents.append(self.getEnt("airlock")(-50,a.pos[1]+(int(a.size[1]/100)*50),self.__LINK,self.__IDCount))
                            self.__IDCount+=1
                        else:
                            self.ents.append(self.getEnt("airlock")(a.pos[0]+(int(a.size[0]/100)*50),-50,self.__LINK,self.__IDCount))
                            self.__IDCount+=1
                        self.ents[-1].editMove(self.ents)
                        RoomDoors[a].append(self.ents[-1])
