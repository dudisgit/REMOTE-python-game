...
class Main(base.Main):
    ...
    def loop(self,lag): #Loop ran single player or server side
        if not self.__noSet and not self.drone is None: #Drone hasn't been sped up before
            self.drone.speed += 1
            self.__changeDrone = self.drone.ID+0
            self.__noSet = True
    def clientLoop(self,lag): #Loop ran client side
        if self.__changeDrone!=self.drone.ID: #Drone attachment has changed, changing speed...
